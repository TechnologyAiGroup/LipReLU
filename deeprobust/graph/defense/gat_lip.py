...
class GAT(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        x, self.attention_1 = self.conv1(x, edge_index, return_attention_weights=True)
        self.layer_1_output = x
        x = F.elu(x)
        x = F.dropout(x, p=self.dropout, training=self.training)
        self.layer_final_input = x
        x, self.attention_2= self.conv2(x, edge_index, return_attention_weights=True)
    ...
    def fit(self, pyg_data, train_iters=300, initialize=True, verbose=False, patience=100, lip_norm=False, lip_node=False, lip_relu=False, gamma=0.00, **kwargs):
        ...
        self._train_with_val(train_iters, lip_norm, lip_node, lip_relu, gamma, verbose)

    def train_with_early_stopping(self, train_iters, patience, lip_norm, lip_node, lip_relu, gamma,  verbose):
    ...
    def _train_with_val(self, train_iters, lip_norm, lip_node, lip_relu, gamma, verbose):
        if verbose:
            print('=== training GAT model ===')
        if verbose:
