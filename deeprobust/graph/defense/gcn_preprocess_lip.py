...
class GCNSVD(GCN):
    ...
    def fit(self, features, adj, labels, idx_train, idx_val=None, k=50, train_iters=200, initialize=True, verbose=True, lip_norm=False, lip_node=False, lip_relu=False, gamma=0.0, **kwargs):
        ...
        self.labels = labels
        super().fit(features, modified_adj, labels, idx_train, idx_val, train_iters=train_iters, initialize=initialize, lip_norm=lip_norm, lip_node=lip_node, lip_relu=lip_relu, gamma=gamma, verbose=verbose)
...
class GCNJaccard(GCN):
    ...
    def fit(self, features, adj, labels, idx_train, idx_val=None, threshold=0.01, train_iters=200, initialize=True, lip_norm=False, lip_node = False, lip_relu = False, gamma= 0.0, verbose=True, **kwargs):
...
if __name__ == "__main__":
    ...
    model.fit(features, perturbed_adj, labels, idx_train, idx_val, threshold=0.1, lip_norm=False, gamma=0.00)
    model.test(idx_test)
    ...
    model = GCNSVD(nfeat=features.shape[1],
          ...
          nclass=labels.max().item() + 1,
          dropout=0.5, device='cuda').to('cuda')
    model.fit(features, perturbed_adj, labels, idx_train, idx_val, k=20, lip_norm=False, gamma=0.00)
    model.test(idx_test)
    prediction_1 = model.predict()
    prediction_2 = model.predict(features, perturbed_adj)
