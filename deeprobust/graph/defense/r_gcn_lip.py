...
class RGCN(Module):
    ...
    def forward(self, features):
        features.data = self.features.data
...
######################
    ...
    def lip_reg(self, features, idx_train):
        lip_mat = []
    ...
    def fit(self, features, adj, labels, idx_train, idx_val=None, train_iters=200, lip_norm=False, lip_node = False, gamma=0.00, verbose=True, **kwargs):
        """Train RGCN.
        ...
        self._initialize()
        if idx_val is None:
            self._train_without_val(adj, features, labels, idx_train, train_iters, lip_norm, lip_node, gamma, verbose)
        else:
            self._train_with_val(adj, features, labels, idx_train, idx_val, train_iters, lip_norm, lip_node, gamma, verbose)

    # def _train_without_val(self, adj, features, labels, idx_train, train_iters, lip_norm, lip_node, gamma, verbose=True):
    ...
    #         if lip_norm == True:
    #             loss_lip = self.lip_reg(features, idx_train)
    #             loss_train = loss_train + gamma * loss_lip
    #         if lip_node == True:
    #             loss_lip = self.lip_regulation_node(features, adj)
    #             loss_train = loss_train + gamma * loss_lip
    #
    #         loss_train.backward()
    #         optimizer.step()
    ...
    def _train_with_val(self, features, adj,  labels, idx_train, idx_val, train_iters, lip_norm, lip_node, gamma, verbose):
        optimizer = optim.Adam(self.parameters(), lr=self.lr)
        ...
        for i in range(train_iters):
            ...
            optimizer.step()
            if verbose and i % 10 == 0:
                print('Epoch {}, training loss: {}, time: {}, lip_constant: {}'.format(i, loss_train.item(), time.time() - start_time, lip_constant))
...
if __name__ == "__main__":
    ...
    model.fit(features, perturbed_adj, labels, idx_train, idx_val,
                      train_iters=200, lip_norm=False, gamma=0.00, verbose=True)
    model.test(idx_test)
