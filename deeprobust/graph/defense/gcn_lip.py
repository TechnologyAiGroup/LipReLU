...
###########################
    ...
    def fit(self, features, adj, labels, idx_train, idx_val=None, train_iters=200, initialize=True, verbose=False, normalize=True, patience=500, lip_norm=False,  lip_node=False, lip_relu=False, gamma= 0.0, **kwargs):
    ...
    def _train_with_val(self, labels, idx_train, idx_val, train_iters, lip_norm, lip_node, lip_relu, gamma, verbose):
        ...
        for i in range(train_iters):
            ...
            if verbose and i % 1 == 0:
                print('Epoch {}, training loss: {}, time: {}, lip_constant: {}, train_acc :{}'.format(i, loss_train.item(), time.time() - start_time, lip_constant, acc_train))
    ...
    def _train_with_early_stopping(self, labels, idx_train, idx_val, train_iters, patience, lip_norm, lip_node, lip_relu, gamma, verbose):
        if verbose:
            print('=== training gcn model ===')
        optimizer = optim.Adam(self.parameters(), lr=self.lr, weight_decay=self.weight_decay)
