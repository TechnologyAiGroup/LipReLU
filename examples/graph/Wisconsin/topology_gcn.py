...
# def get_data_split():
#
#     idx_train = np.array(range(600))
#     idx_val = np.array(range(600,1500))
#     idx_test = np.array(range(1500, 2000))
#     return idx_train, idx_val, idx_test
...
path_2 = "examples/graph/OtherDataset/topology_modified_adj/wisconsin_ptb_0.25.pt"
#
...
def test(features, adj, labels):
    ...
    gcn = gcn.to(device)
    gcn.fit(features, adj, labels, idx_train, idx_val, train_iters=200, lip_norm=True, lip_node=False, lip_relu=False,
            gamma=0.09, verbose=True)  # train without model picking
